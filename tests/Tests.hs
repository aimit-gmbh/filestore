...
main = do
  ...
  let
    counts = Counts
      { cases    = cases gc {- + cases dc -}
                            + cases mc
      , tried    = tried gc {- + tried dc -}
                            + tried mc
      , failures = failures gc {- + failures dc -}
                               + failures mc
      , errors   = errors gc {- + errors dc -}
                             + errors mc
      }
  let errorCode = failures counts + errors counts
  putStrLn $ "Total " ++ showCounts counts
  exitWith $ if errorCode == 0 then ExitSuccess else ExitFailure errorCode

testFileStore :: FileStore -> FileStoreType -> IO Counts
testFileStore fs fsName = runTestTT $ TestList $ map
  (\(label, testFn) ->
...
preInitializeTest fs = TestCase $ do
      ...
      (do
        index fs
        assertFailure "preInitialize, uncaught error"
      )
    $ \e -> assertEqual
        "error status from attempt to get index of nonexistent repo"
        e
        NotFound
  catch
      (do
    ...
    $ \e -> assertEqual
        "error status from attempt to get directory of nonexistent repo"
        e
        NotFound
...
initializeTest fs = TestCase $ do
  ...
  assertEqual "index of just-initialized repository" ind []
      ...
      (  initialize fs
      >> assertFailure "did not return error for existing repository"
      )
    $ \e -> assertEqual
        "error status from existing repository"
        e
        RepositoryExists
...
createTest3 fs = TestCase $ do
  ...
  allfiles <- index fs
  assertBool
    "index contains file with nonascii name"
    (nonasciiTestTitle `elem` allfiles)

createTest3a fs = TestCase $ do
  create
    fs
    subdirNonasciiTestTitle
    testAuthor
  ...
  allfiles <- index fs
  assertBool
    "index contains file with nonascii subdir"
    (subdirNonasciiTestTitle `elem` allfiles)
...
createTest4 fs = TestCase $ do
      ...
      (  create fs "../oops" testAuthor "description of change" testContents
...
createTest5 fsName fs = TestCase $ do
  let
    (realpath, special) = case fsName of
      Git -> ("tmp" </> "gitfs" </> ".git" </> "newfile", ".git/newfile")
      Darcs ->
        ("tmp" </> "darcsfs" </> "_darcs" </> "newfile", "_darcs/newfile")
      Mercurial ->
        ("tmp" </> "mercurialfs" </> ".hg" </> "newfile", ".hg/newfile")
  catch
      (  create fs special testAuthor "description of change" testContents
  ...
  assertBool
    ("file " ++ realpath ++ " was created outside repository")
    (not exists)
...
directoryTest fs = TestCase $ do
  ...
  files <- directory fs ""
  assertEqual
    "result of directory on top-level"
    (sort
      [ FSDirectory (takeDirectory subdirTestTitle)
      , FSFile testTitle
      , FSFile nonasciiTestTitle
      , FSDirectory (takeDirectory subdirNonasciiTestTitle)
      ]
    )
    (sort files)
  ...
  assertEqual
    "result of directory on subdir"
    [ FSFile (takeFileName subdirTestTitle)
    , FSFile (takeFileName (subdirTestTitle ++ "2"))
    ]
    subdirFiles
  ...
  -- Try to get contents of nonexistent subdirectory
      ...
      (do
        directory fs "foo"
        assertFailure "nonexistent subdirectory, uncaught error"
      )
    $ \e -> assertEqual
        "error status from attempt to get directory listing of nonexistent directory"
        e
        NotFound
...
retrieveTest4 fs =
  TestCase
    $ catch
        (  (retrieve fs "subdir" Nothing :: IO String)
...
modifyTest fs = TestCase $ do
  ...
  assertEqual
    "retrieved contents after modify"
    modifiedContents'
    modifiedContents
  ...
  modResult2 <- modify
    ...
    (testContents ++ "\nFourth line")
  let
    normModResult2 = Left MergeInfo
      { mergeRevision = newRev
      , mergeConflicts = True
      , mergeText = "Test contents.\nSecond line.\n<<<<<<< edited\nThird test line with some Greek \945\946.\nFourth line\n=======\n>>>>>>> "
        ++ newRevId
        ++ "\n"
      }
  ...
  modResult3 <- modify
    ...
    modifiedContents
  let
    normModResult3 = Left MergeInfo
      { mergeRevision  = newRev
      , mergeConflicts = False
      , mergeText      = modifiedContents
      }
  assertEqual
    "results of modify from old version with new version's contents"
    normModResult3
    modResult3
...
deleteTest fsName fs = TestCase $ do
  ...
  ind <- index fs
  assertBool
    "index does not contain resource that was deleted"
    (toBeDeleted `notElem` ind)
  ...
  let
    (realpath, special) = case fsName of
      ...
      (  delete fs special testAuthor "description of change"
...
renameTest fs = TestCase $ do
  ...
  -- Try renaming a file that doesn't exist.
      ...
      (  rename fs "nonexistent file" "other name" testAuthor "rename"
      >> assertFailure "rename of nonexistent file did not throw error"
      )
    $ \e -> assertEqual
        "error status from rename of nonexistent file"
        NotFound
        e
  ...
  let cmd     = "rename " ++ newName ++ " " ++ badName
      ...
      (  rename fs newName badName testAuthor "description of change"
...
historyTest fs = TestCase $ do
  ...
  hist <- history
    fs
    [testTitle, subdirTestTitle, nonasciiTestTitle]
  ...
  histNow <- history
    fs
    [testTitle]
    (TimeRange (Just $ addUTCTime (60 * 60 * 24) now) Nothing)
    Nothing
...
diffTest fs = TestCase $ do
  ...
  save
    ...
    testAuthor
    "removed a line"
    (unlines . init . lines $ testContents)

  [secondrev, firstrev] <- history
    fs
    [diffTitle]
    (TimeRange Nothing Nothing)
    Nothing
  ...
  assertEqual
    "added lines from empty document to first revision"
    (lines testContents)
    added''
...
searchTest fs = TestCase $ do
  ...
  assertEqual
    "search results 1"
    [SearchMatch "bar" 1 "bing BONG", SearchMatch "foo" 1 "bing"]
    res1
  ...
  assertEqual
    "search results 3"
    [SearchMatch "foo" 2 "bong", SearchMatch "foo" 4 "φ"]
    res3
  ...
  assertEqual
    "search results 4"
    [ SearchMatch "bar" 1 "bing BONG"
    , SearchMatch "baz" 2 "bong"
    , SearchMatch "foo" 2 "bong"
    , SearchMatch "foo" 4 "φ"
    ]
    res4
  ...
  assertEqual
    "search results 5"
    [ SearchMatch "bar" 1 "bing BONG"
    , SearchMatch "baz" 1 "bingbang"
    , SearchMatch "foo" 1 "bing"
    ]
    res5
...
matchTest fs = TestCase $ do
  ...
  assertBool
    "match with nonidentical but matching IDs"
    (idsMatch fs "abcde" "abcde5553")
