...
runGitCommand
  :: FilePath -> String -> [String] -> IO (ExitCode, String, B.ByteString)
...
gitCommit repo names author logMsg = do
  ...
  (statusCommit, errCommit, _) <-
    ...
    $  [ "--author"
       , authorName author ++ " <" ++ authorEmail author ++ ">"
       , "-m"
       , logMsg
       ]
    ++ names
...
gitSave repo name author logMsg contents = do
  ...
  if statusAdd == ExitSuccess
    then gitCommit repo [name] author logMsg
      ...
      throwIO $ UnknownError $ "Could not git add '" ++ name ++ "'\n" ++ errAdd
...
gitMove repo oldName newName author logMsg = do
  ...
  (statusAdd, err, _) <- withSanityCheck repo [".git"] newName
    $ runGitCommand repo "mv" [oldName, newName]
  if statusAdd == ExitSuccess
...
gitLatestRevId repo name = do
  (revListStatus, _, output) <- runGitCommand
    repo
    "rev-list"
    ["--max-count=1", "HEAD", "--", name]
  ...
  if revListStatus == ExitSuccess && catStatus == ExitSuccess
    then do
      ...
      if null result then throwIO NotFound else return result
...
gitGetRevision repo revid = do
  (status, _, output) <- runGitCommand
    repo
    "whatchanged"
    ["-z", "--pretty=format:" ++ gitLogFormat, "--max-count=1", revid]
  if status == ExitSuccess
...
gitIndex repo = withVerifyDir repo $ do
 ...
 where
  lineToFilename (_ : "blob" : _ : rest) = Just $ unwords rest
...
gitDirectory repo dir = withVerifyDir (repo </> dir) $ do
  ...
  if status == ExitSuccess
    then return $ map (lineToResource . words) $ endByOneOf ['\0'] $ B.unpack
      output
    else return []   -- if error, this will return empty list
                      -- note:  on a newly initialized repo, 'git ls-tree HEAD:' returns an error
 where
  lineToResource (_ : "blob" : _ : rest) = FSFile $ unwords rest
...
gitSearch repo query = do
  ...
  case status of
    ...
    ExitFailure _ ->
      throwIO $ UnknownError $ "git grep returned error status.\n" ++ errOutput

-- Auxiliary function for searchResults
parseMatchLine :: String -> SearchMatch
parseMatchLine str = SearchMatch
...
gitLog repo names (TimeRange mbSince mbUntil) mblimit = do
  (status, err, output) <-
    runGitCommand repo "whatchanged"
  ...
  if status == ExitSuccess
    then parseGitLog output
      ...
      throwIO $ UnknownError $ "git whatchanged returned error status.\n" ++ err
...
parseLogEntry entry = do
  ...
  return Revision
    ...
    , revAuthor      = Author
      { authorName  = B.unpack author
      , authorEmail = B.unpack email
      }
    , revDescription = B.unpack $ stripTrailingNewlines subject
    , revChanges     = changes
    }
...
parseChanges (x : y : zs) = do
  ...
  let file'      = B.unpack y
  if changeType == 'R'
    then parseChanges (tail zs) >>= return . (++)
      (Deleted file' : Added (B.unpack $ head zs) : [])
    else do
      next <- case changeType of
        'A' -> return $ Added file'
      ...
      return (next : rest)
parseChanges [_] = pcErr "encountered odd number of fields"
parseChanges []  = return []
