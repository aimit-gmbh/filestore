...
mercurialCommit repo names author logMsg = do
  let email  = authorEmail author
      email' = if not (null email) then " <" ++ email ++ ">" else ""
  (statusCommit, errCommit, _) <-
    runMercurialCommand repo "commit"
    $  ["--user", authorName author ++ email', "-m", logMsg]
    ++ names
...
mercurialSave
  :: Contents a => FilePath -> FilePath -> Author -> Description -> a -> IO ()
mercurialSave repo name author logMsg contents = do
  withSanityCheck repo [".hg"] name
    $ B.writeFile (repo </> encodeArg name)
    $ toByteString contents
  ...
  if statusAdd == ExitSuccess
    then mercurialCommit repo [name] author logMsg
      ...
      throwIO $ UnknownError $ "Could not hg add '" ++ name ++ "'\n" ++ errAdd
...
mercurialRetrieve repo name revid = do
  let revname = case revid of
        Nothing  -> "tip"
        Just rev -> rev
  (statcheck, _, _) <- runMercurialCommand
    repo
    "locate"
    ["-r", revname, "-X", "glob:" ++ name </> "*", "path:" ++ name]
  when (statcheck /= ExitSuccess) $ throwIO NotFound
  (status, err, output) <- runMercurialCommand
    repo
    "cat"
    ["-r", revname, "-X", "glob:" ++ name </> "*", "path:" ++ name]
  if status == ExitSuccess
...
mercurialDelete repo name author logMsg =
  withSanityCheck repo [".hg"] name $ do
    (statusAdd, errRm, _) <- runMercurialCommand repo "remove" ["path:" ++ name]
    if statusAdd == ExitSuccess
      then mercurialCommit repo [name] author logMsg
      else
        throwIO $ UnknownError $ "Could not hg rm '" ++ name ++ "'\n" ++ errRm

-- | Change the name of a resource.
mercurialMove
  :: FilePath -> FilePath -> FilePath -> Author -> Description -> IO ()
mercurialMove repo oldName newName author logMsg = do
  ...
  (statusAdd, err, _) <-
    withSanityCheck repo [".hg"] newName
      $ runMercurialCommand repo "mv" [oldName, newName]
  if statusAdd == ExitSuccess
...
mercurialLatestRevId repo name = do
  (status, _, output) <- runMercurialCommand
    repo
    "log"
    [ "--template"
    , "{node}\\n{file_dels}\\n"
  ...
  if status == ExitSuccess
    then do
      let result = lines $ unpack output
...
mercurialGetRevision repo revid = do
  (status, _, output) <- runMercurialCommand
    repo
    "log"
    ["--template", mercurialLogFormat, "--limit", "1", "-r", revid]
  if status == ExitSuccess
    then case P.parse parseMercurialLog "" (unpack output) of
      Left err' ->
        throwIO $ UnknownError $ "error parsing mercurial log: " ++ show err'
      Right [r] -> return r
      Right []  -> throwIO NotFound
      Right xs ->
        throwIO
          $  UnknownError
          $  "mercurial log returned more than one result: "
          ++ show xs
    else throwIO NotFound
...
mercurialIndex repo = withVerifyDir repo $ do
  ...
  if status == ExitSuccess
    then return $ lines $ unpack $ output
...
mercurialDirectory repo dir = withVerifyDir (repo </> dir) $ do
  ...
  let files = if status == ExitSuccess
        then map (FSFile . takeFileName . removePrefix dir) $ lines $ unpack
          output
        else []
  (status2, _, output2) <- runMercurialCommand
    repo
    "locate"
    ["-r", "tip", "glob:" ++ (dir </> "*" </> "*")]
  let dirs = if status2 == ExitSuccess
        then
          map FSDirectory
          $ nub
          $ map (head . splitDirectories . removePrefix dir)
          $ lines
          $ unpack output2
        else []
  return $ files ++ dirs
...
mercurialSearch repo query = do
  ...
  case status of
     ExitSuccess   -> do
                       ...
                       case P.parse parseMercurialSearch "" (unpack output) of
...
mercurialLog
  :: FilePath -> [FilePath] -> TimeRange -> Maybe Int -> IO [Revision]
mercurialLog repo names (TimeRange mbSince mbUntil) mblimit = do
  (status, err, output) <-
    runMercurialCommand repo "log"
    $  ["--template", mercurialLogFormat]
    ++ revOpts mbSince mbUntil
    ++ limit
    ++ names
  if status == ExitSuccess
    then case P.parse parseMercurialLog "" (unpack output) of
      Left err' ->
        throwIO $ UnknownError $ "Error parsing mercurial log.\n" ++ show err'
      Right parsed -> return parsed
    else
      throwIO $ UnknownError $ "mercurial log returned error status.\n" ++ err
 where
...
mercurialLogEntry = do
  ...
  return Revision
    { revId          = rev
    , revDateTime    = fromJust
      (parseTime defaultTimeLocale "%a, %d %b %Y %H:%M:%S %z" date :: Maybe
          UTCTime
      )
